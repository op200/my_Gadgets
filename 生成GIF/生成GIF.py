...
set_outScale_Checkbutton = ttk.Checkbutton(
    set_outScale_Frame,
    text="重缩放",
    var=set_outScale_Tkbool,  # type: ignore
    command=set_outScale_Checkbutton_Click,
)
...
set_outPTS_Checkbutton = ttk.Checkbutton(
    set_outPTS_Frame,
    text="重设PTS倍率",
    var=set_outPTS_Tkbool,  # type: ignore
...
def end_to_ready():
    ...
    start_createGIF_Button.config(
        text="生成GIF", command=start_to_ready, state=tk.NORMAL
    )
...
def Thread_encoding():
    ...
    set_scale = (
        f"scale={set_outScale_width_Entry.get()}x{set_outScale_height_Entry.get()},"
        if set_outScale_Tkbool.get()
        else ""
    )

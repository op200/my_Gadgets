...
def srt_to_lrc(srt_file, lrc_file):
    ...
    with (
        ...
        open(lrc_file, "w", encoding="utf-8") as lrc,
    ):
        for line in srt:
            line = line.strip()
            if not line:
            ...
            if match:
                # 解析时间戳
                h1, m1, s1, ms1, h2, m2, s2, ms2 = match.groups()  # noqa: RUF059
...
class log:
    ...
    @staticmethod
    def errore(info: object) -> NoReturn:
        log.error(info)
        log.exit()
...
if output_path[-4:] != ".lrc":
    log.warning(
        "The suffix of the output file is not '.lrc' and has been automatically corrected"
    )
    output_path += ".lrc"
...
if not overwrite_lrc:
    if os.path.exists(output_path):
        ...
        while conf_overwrite := input():
            if conf_overwrite in {"y", "Y"}:
                break
            elif conf_overwrite in {"n", "N"}:
